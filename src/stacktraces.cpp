...
static __attribute__((noinline)) void DoCrash2()
{
    ...
    switch (rnd) {
        case 0:
            PrintCrashInfo(strprintf("%s: throwing exception!\n", __func__));
            throw std::runtime_error("whoops");
        case 1:
            PrintCrashInfo(strprintf("%s: asserting %d==2!\n", __func__, rnd));
            assert(rnd == 2);
            break;
        case 2: {
            PrintCrashInfo(strprintf("%s: accessing invalid memory!\n", __func__));
            int* p = (int*) 0x1;
            *p = 1;
            break;
        }
        case 3: {
            PrintCrashInfo(strprintf("%s: dividing by zero!\n", __func__));
            test = test / zero;
            break;
        }
        case 4: {
            PrintCrashInfo(strprintf("%s: calling std::terminate()!\n", __func__));
            std::terminate();
            break;
        }
        case 5: {
            PrintCrashInfo(strprintf("%s: causing stack overflow!\n", __func__));
