...
CDKGLogger::CDKGLogger(const CDKGSession& _quorumDkg, const std::string& _func) :
    CDKGLogger(_quorumDkg.params.name, _quorumDkg.pquorumBaseBlockIndex->GetBlockHash(), _quorumDkg.pquorumBaseBlockIndex->nHeight, _quorumDkg.AreWeMember(), _func)
{
}
...
bool CDKGSession::Init(const CBlockIndex* _pquorumBaseBlockIndex, const std::vector<CDeterministicMNCPtr>& mns, const uint256& _myProTxHash)
{
    ...
    if (!myProTxHash.IsNull()) {
        quorumDKGDebugManager->InitLocalSessionStatus(params.type, pquorumBaseBlockIndex->GetBlockHash(), pquorumBaseBlockIndex->nHeight);
        relayMembers = CLLMQUtils::GetQuorumRelayMembers(params.type, pquorumBaseBlockIndex, myProTxHash, true);
    }
...
void CDKGSession::SendContributions(CDKGPendingMessages& pendingMessages)
{
    ...
    qc.quorumHash = pquorumBaseBlockIndex->GetBlockHash();
...
bool CDKGSession::PreVerifyMessage(const CDKGContribution& qc, bool& retBan) const
{
    ...
    if (qc.quorumHash != pquorumBaseBlockIndex->GetBlockHash()) {
...
void CDKGSession::ReceiveMessage(const CDKGContribution& qc, bool& retBan)
{
    ...
    dkgManager.WriteVerifiedVvecContribution(params.type, pquorumBaseBlockIndex, qc.proTxHash, qc.vvec);
...
void CDKGSession::VerifyPendingContributions()
{
    ...
    for (const auto& idx : pend) {
        ...
        dkgManager.WriteEncryptedContributions(params.type, pquorumBaseBlockIndex, m->dmn->proTxHash, *vecEncryptedContributions[idx]);
    }
    ...
    for (size_t i = 0; i < memberIndexes.size(); i++) {
        if (!result[i]) {
            ...
            dkgManager.WriteVerifiedSkContribution(params.type, pquorumBaseBlockIndex, members[memberIdx]->dmn->proTxHash, skContributions[i]);
        }
    }
...
void CDKGSession::SendJustification(CDKGPendingMessages& pendingMessages, const std::set<uint256>& forMembers)
{
    ...
    qj.quorumHash = pquorumBaseBlockIndex->GetBlockHash();
...
bool CDKGSession::PreVerifyMessage(const CDKGJustification& qj, bool& retBan) const
{
    ...
    if (qj.quorumHash != pquorumBaseBlockIndex->GetBlockHash()) {
    ...
    for (const auto& p : qj.contributions) {
        ...
        if (!result) {
            ...
            if (AreWeMember() && member2->id == myId) {
                ...
                dkgManager.WriteVerifiedSkContribution(params.type, pquorumBaseBlockIndex, member->dmn->proTxHash, skContribution);
            }
...
void CDKGSession::SendCommitment(CDKGPendingMessages& pendingMessages)
{
    ...
    std::vector<BLSVerificationVectorPtr> vvecs;
    BLSSecretKeyVector skContributions;
    if (!dkgManager.GetVerifiedContributions(params.type, pquorumBaseBlockIndex, qc.validMembers, memberIndexes, vvecs, skContributions)) {
...
std::vector<CFinalCommitment> CDKGSession::FinalizeCommitments()
{
    ...
    for (const auto& p : commitmentsMap) {
        ...
        cxxtimer::Timer t3(true);
        if (!fqc.Verify(pquorumBaseBlockIndex, true)) {
