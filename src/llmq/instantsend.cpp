...
CInstantSendLockPtr CInstantSendDb::GetInstantSendLockByHash(const uint256& hash, bool use_cache) const
{
    ...
    if (bool exists = db->Read(std::make_tuple(DB_ISLOCK_BY_HASH, hash), *ret); !exists || (::SerializeHash(*ret) != hash)) {
...
void CInstantSendManager::ProcessTx(const CTransaction& tx, bool fRetroactive, const Consensus::Params& params)
{
    ...
    if (auto conflictingLock = GetConflictingLock(tx); conflictingLock != nullptr) {
...
bool CInstantSendManager::TrySignInputLocks(const CTransaction& tx, bool fRetroactive, Consensus::LLMQType llmqType)
{
    ...
    for (const auto& in : tx.vin) {
        ...
        if (uint256 otherTxHash; quorumSigningManager->GetVoteForId(llmqType, id, otherTxHash)) {
...
bool CInstantSendManager::CheckCanLock(const COutPoint& outpoint, bool printDebug, const uint256& txHash, const Consensus::Params& params) const
{
    ...
    if (auto mempoolTx = mempool.get(outpoint.hash); mempoolTx) {
        if (printDebug) {
...
void CInstantSendManager::HandleNewInputLockRecoveredSig(const CRecoveredSig& recoveredSig, const uint256& txid)
{
    ...
    CTransactionRef tx;
    if (uint256 hashBlock; !GetTransaction(txid, tx, Params().GetConsensus(), hashBlock)) {
        return;
    }
...
bool CInstantSendManager::ProcessPendingInstantSendLocks()
{
    ...
    if (auto badISLocks = ProcessPendingInstantSendLocks(0, pend, false);
            !badISLocks.empty()) {
...
void CInstantSendManager::UpdatedBlockTip(const CBlockIndex* pindexNew)
{
    ...
    if (bool fDIP0008Active = pindexNew->pprev && pindexNew->pprev->nHeight >= Params().GetConsensus().DIP0008Height;
            AreChainLocksEnabled() && fDIP0008Active) {
