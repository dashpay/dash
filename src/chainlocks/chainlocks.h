...
#ifndef BITCOIN_CHAINLOCKS_CHAINLOCKS_H
#define BITCOIN_CHAINLOCKS_CHAINLOCKS_H
...
private:
    ...
    std::unique_ptr<chainlocks::ChainLockSigner> m_signer{nullptr};
    ...
    chainlocks::ChainLockSig bestChainLock GUARDED_BY(cs);

    chainlocks::ChainLockSig bestChainLockWithKnownBlock GUARDED_BY(cs);
...
public:
    ...
    bool GetChainLockByHash(const uint256& hash, chainlocks::ChainLockSig& ret) const EXCLUSIVE_LOCKS_REQUIRED(!cs);
    chainlocks::ChainLockSig GetBestChainLock() const EXCLUSIVE_LOCKS_REQUIRED(!cs);

    [[nodiscard]] MessageProcessingResult ProcessNewChainLock(NodeId from, const chainlocks::ChainLockSig& clsig,
    ...
    VerifyRecSigStatus VerifyChainLock(const chainlocks::ChainLockSig& clsig) const;
...
private:
    ...
    friend class ::chainlocks::ChainLockSigner;
};

bool AreChainLocksEnabled(const CSporkManager& sporkman);
} // namespace llmq

#endif // BITCOIN_CHAINLOCKS_CHAINLOCKS_H
