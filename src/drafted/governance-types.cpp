...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "KliqNetwork") type = KliqNetwork;
    if(strType == "KliqNetworkVariable") type = KliqNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == KliqNetwork) s = "KliqNetwork";
    if(type == KliqNetworkVariable) s = "KliqNetworkVariable";
