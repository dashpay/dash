...
void CMasternodeUtils::ProcessMasternodeConnections(CConnman& connman)
{
    ...
    connman.ForEachNode(CConnman::AllNodes, [&](CNode* pnode) {
        if (!pnode->fInbound && !pnode->fFeeler && !pnode->m_manual_connection && !pnode->m_masternode_connection && !pnode->m_probe_connection) {
            nonMasternodeCount++;
        }
    });
    ...
    connman.ForEachNode(CConnman::AllNodes, [&](CNode* pnode) {
        ...
        if (pnode->m_probe_connection && GetSystemTimeInSeconds() - pnode->nTimeConnected < 5) return;
