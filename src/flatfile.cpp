...
FILE* FlatFileSeq::Open(const FlatFilePos& pos, bool read_only)
{
    ...
    if (pos.nPos && fseek(file, pos.nPos, SEEK_SET)) {
        ...
        if (fclose(file) != 0) {
            LogError("Unable to close file %s after seek failure: %s", fs::PathToString(path), SysErrorString(errno));
        }
        return nullptr;
    }
    return file;
...
bool FlatFileSeq::Flush(const FlatFilePos& pos, bool finalize)
{
    ...
    if (finalize && !TruncateFile(file, pos.nPos)) {
        if (fclose(file) != 0) {
            LogError("%s: failed to close file %d after truncate failure: %s", __func__, pos.nFile, SysErrorString(errno));
        }
        return error("%s: failed to truncate file %d", __func__, pos.nFile);
    }
    if (!FileCommit(file)) {
        if (fclose(file) != 0) {
            LogError("%s: failed to close file %d after commit failure: %s", __func__, pos.nFile, SysErrorString(errno));
        }
    ...
    if (fclose(file) != 0) {
        return error("%s: failed to close file %d: %s", __func__, pos.nFile, SysErrorString(errno));
    }
    return true;
}
