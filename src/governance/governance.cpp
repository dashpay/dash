                                                ...
                                                const CActiveMasternodeManager& mn_activeman)
{
    ...
    for (const auto& trigger : activeTriggers) {
        const auto govobj = FindGovernanceObject(trigger->GetHash());
...
bool CGovernanceManager::IsSuperblockTriggered(const CDeterministicMNList& tip_mn_list, int nBlockHeight)
{
    ...
    for (const auto& pSuperblock : vecTriggers) {
        ...
        CGovernanceObject* pObj = FindGovernanceObject(pSuperblock->GetHash());
        if (!pObj) {
        ...
        if (nBlockHeight != pSuperblock->GetBlockHeight()) {
            LogPrint(BCLog::GOBJECT, "IsSuperblockTriggered -- block height doesn't match nBlockHeight = %d, blockStart = %d, continuing\n",
                nBlockHeight,
...
bool CGovernanceManager::GetBestSuperblock(const CDeterministicMNList& tip_mn_list, CSuperblock_sptr& pSuperblockRet, int nBlockHeight)
{
    ...
    for (const auto& pSuperblock : vecTriggers) {
        ...
        const CGovernanceObject* pObj = FindGovernanceObject(pSuperblock->GetHash());
        if (!pObj) {
            continue;
        }
...
bool CGovernanceManager::GetSuperblockPayments(const CDeterministicMNList& tip_mn_list, int nBlockHeight, std::vector<CTxOut>& voutSuperblockRet)
{
    LOCK(cs);
...
bool CGovernanceManager::IsValidSuperblock(const CChain& active_chain, const CDeterministicMNList& tip_mn_list, const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
