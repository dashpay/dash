...
struct TestChainDATSetup : public TestChainSetup
{
    ...
    void test(int activation_index, bool check_activation_at_min)
    {
...

        {
            ...
            const auto pblocktemplate = BlockAssembler(*sporkManager, *governance, *m_node.llmq_ctx->quorum_block_processor, *m_node.llmq_ctx->clhandler, *m_node.llmq_ctx->isman, g_evoDb, *m_node.mempool, Params()).CreateNewBlock(coinbasePubKey);
