...
BOOST_AUTO_TEST_CASE(key_test1)
{
    ...
    for (int n=0; n<16; n++)
    {
        ...
        auto opt_sign1 = key1.SignCompact (hashMsg);
        auto opt_sign2 = key2.SignCompact (hashMsg);
        auto opt_sign1c = key1C.SignCompact(hashMsg);
        auto opt_sign2c = key2C.SignCompact(hashMsg);

        BOOST_CHECK(opt_sign1);
        BOOST_CHECK(opt_sign2);
        BOOST_CHECK(opt_sign1c);
        BOOST_CHECK(opt_sign2c);

        CPubKey rkey1, rkey2, rkey1C, rkey2C;

        BOOST_CHECK(rkey1.RecoverCompact (hashMsg, *opt_sign1));
        BOOST_CHECK(rkey2.RecoverCompact (hashMsg, *opt_sign2));
        BOOST_CHECK(rkey1C.RecoverCompact(hashMsg, *opt_sign1c));
        BOOST_CHECK(rkey2C.RecoverCompact(hashMsg, *opt_sign2c));
    ...
    auto opt_detsig = key1.SignCompact(hashMsg);
    auto opt_detsigc = key1C.SignCompact(hashMsg);

    BOOST_CHECK(opt_detsig);
    BOOST_CHECK(opt_detsigc);
    detsig = std::vector<uint8_t>{opt_detsig->begin(), opt_detsig->end()};
    detsigc = std::vector<uint8_t>{opt_detsigc->begin(), opt_detsigc->end()};
    BOOST_CHECK(detsig == ParseHex("1c5dbbddda71772d95ce91cd2d14b592cfbc1dd0aabd6a394b6c2d377bbe59d31d14ddda21494a4e221f0824f0b8b924c43fa43c0ad57dccdaa11f81a6bd4582f6"));
    BOOST_CHECK(detsigc == ParseHex("205dbbddda71772d95ce91cd2d14b592cfbc1dd0aabd6a394b6c2d377bbe59d31d14ddda21494a4e221f0824f0b8b924c43fa43c0ad57dccdaa11f81a6bd4582f6"));
    opt_detsig = key2.SignCompact(hashMsg);
    opt_detsigc = key2C.SignCompact(hashMsg);
    BOOST_CHECK(opt_detsig.has_value());
    BOOST_CHECK(opt_detsigc.has_value());
    detsig = std::vector<uint8_t>{opt_detsig->begin(), opt_detsig->end()};
    detsigc = std::vector<uint8_t>{opt_detsigc->begin(), opt_detsigc->end()};
