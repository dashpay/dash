...
static std::vector<unsigned char> CanonicalOperatorKeyBytes(const CBLSPublicKey& public_key)
{
...
MasternodeOperatorKeyStatus CWallet::TopUpMasternodeOperatorLookahead()
{
    ...
    ScriptPubKeyMan* source{GetMasternodeOperatorKeySource(&source_id)};
    ...
    const uint32_t end{std::min<uint32_t>(next + MASTERNODE_OPERATOR_GAP_LIMIT, MASTERNODE_OPERATOR_MAX_INDEX)};
    if (next >= end) return MasternodeOperatorKeyStatus::EXHAUSTED;
    if (m_mn_operator_lookahead && m_mn_operator_lookahead->source_id == source_id &&
        m_mn_operator_lookahead->base_index == next && m_mn_operator_lookahead->public_keys.size() == end - next) {
        return MasternodeOperatorKeyStatus::SUCCESS;
    }

    MasternodeOperatorLookahead lookahead{source_id, next, {}};
    lookahead.public_keys.reserve(end - next);
    const auto status{source->DeriveMasternodeOperatorKeys(MasternodeOperatorCoinType(), next, end,
    ...
    if (lookahead.public_keys.size() != end - next) return MasternodeOperatorKeyStatus::DERIVATION_ERROR;
    ...
    const std::function<bool(const CBLSPublicKey&)>& is_in_use)
{
    ...
    if (is_in_use) {
        ...
        while (consecutive_misses < MASTERNODE_OPERATOR_GAP_LIMIT) {
            ...
            {
                LOCK(cs_wallet);
                ScriptPubKeyMan* source{GetMasternodeOperatorKeySource()};
                if (!source) {
                    result.status = MasternodeOperatorKeyStatus::NOT_SUPPORTED;
                    return result;
                }
