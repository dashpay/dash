...
static void add_coin(CoinsResult& available_coins, CWallet& wallet, const CAmount& nValue, CFeeRate feerate = CFeeRate(0), int nAge = 6*24, bool fIsFromMe = false, int nInput =0, bool spendable = false, int custom_size = 0)
{
    ...
    const auto& coin = available_coins.legacy.emplace_back(COutPoint(wtx.GetHash(), nInput), txout, nAge, custom_size == 0 ? CalculateMaximumSignedInputSize(txout, &wallet, /*coin_control=*/nullptr) : custom_size, /*spendable=*/ true, /*solvable=*/ true, /*safe=*/ true, wtx.GetTxTime(), fIsFromMe, feerate);
    available_coins.total_amount += coin.txout.nValue;
    if (coin.HasEffectiveValue()) {
        available_coins.total_effective_amount = available_coins.total_effective_amount.has_value() ?
                *available_coins.total_effective_amount + coin.GetEffectiveValue() : coin.GetEffectiveValue();
    }
}
...
inline std::vector<OutputGroup>& KnapsackGroupOutputs(const CoinsResult& available_coins, CWallet& wallet, const CoinEligibilityFilter& filter)
{
    ...
    static_groups = GroupOutputs(wallet, available_coins.all(), coin_selection_params, filter);
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        std::unique_ptr<CWallet> dummyWallet = std::make_unique<CWallet>(m_node.chain.get(), /*coinjoin_loader=*/nullptr, "dummy", m_args, CreateMockableWalletDatabase());
                                                          ...
                                                          std::function<CoinsResult(CWallet&)> coin_setup)
{
    ...
    auto groups = GroupOutputs(*wallet, available_coins.all(), cs_params, filter);
    return wallet::SelectCoinsSRD(groups.positive_group, target, cs_params.rng_fast, max_weight);
}
