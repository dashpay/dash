...
{
    // Save and restore process-global CoinJoin options. ForceSetArg keys are
    // stored without the leading '-', so ForceRemoveArg must match that form.
    const bool was_enabled{CCoinJoinClientOptions::IsEnabled()};
    const bool was_multisession{CCoinJoinClientOptions::IsMultiSessionEnabled()};
    const int was_rounds{CCoinJoinClientOptions::GetRounds()};
    const int was_amount{CCoinJoinClientOptions::GetAmount()};
    ...
    gArgs.ForceRemoveArg("enablecoinjoin");
    CCoinJoinClientOptions::SetEnabled(was_enabled);
    CCoinJoinClientOptions::SetMultiSessionEnabled(was_multisession);
    CCoinJoinClientOptions::SetRounds(was_rounds);
    CCoinJoinClientOptions::SetAmount(was_amount);
}
...
{
    // Ensure a known baseline: previous cases must not leave autostart enabled.
    BOOST_CHECK_EQUAL(gArgs.GetBoolArg("-coinjoinautostart", DEFAULT_COINJOIN_AUTOSTART), DEFAULT_COINJOIN_AUTOSTART);
...
BOOST_FIXTURE_TEST_CASE(coinjoin_duplicate_wallet_add_does_not_reinitialize_tests, CTransactionBuilderTestSetup)
{
    const bool was_enabled{CCoinJoinClientOptions::IsEnabled()};
    CCoinJoinClientOptions::SetEnabled(true);
    gArgs.ForceSetArg("-coinjoinautostart", "1");

    bool mixing{true};
    BOOST_CHECK(m_node.cj_walletman->doForClient("", [&](auto& cj_man) {
        cj_man.stopMixing();
        mixing = cj_man.isMixing();
    }));
    BOOST_CHECK_EQUAL(mixing, false);

    m_node.coinjoin_loader->AddWallet(wallet);
    BOOST_CHECK(m_node.cj_walletman->doForClient("", [&](auto& cj_man) { mixing = cj_man.isMixing(); }));

    // ForceSetArg stores under SettingName() (no leading '-'); ForceRemoveArg
    // must use the same key or the forced setting leaks into later fixtures and
    // causes them to autostart mixing.
    gArgs.ForceRemoveArg("coinjoinautostart");
    CCoinJoinClientOptions::SetEnabled(was_enabled);
    BOOST_CHECK_EQUAL(mixing, false);
}
