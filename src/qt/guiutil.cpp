...
{
    // return if URI is not valid or is no jast: URI
    if(!uri.isValid() || uri.scheme() != QString("jast"))
        return false;
...
{
    // Convert jast:// to jast:
    //
    //    Cannot handle this later, because jast:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("jast://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "jast:");
    }
...
{
    QString ret = QString("jast:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open jast.conf with the associated application */
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "jast.desktop";
    return GetAutostartDir() / strprintf("jast-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a jast.desktop file to the autostart directory:
