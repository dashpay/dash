...
void IdentityFlow::broadcastIdentityCreate()
{
    ...
    auto built{platform::st::BuildIdentityCreate(m_service.client().sdk(), proof, keys, asset_lock_signer)};
    if (!built.ok()) {
...
void IdentityFlow::broadcastPreorder()
{
    ...
    m_service.client().getIdentityContractNonce(m_record.identity_id, platform::DPNS_CONTRACT_ID,
        ...
        self->m_service.post([self, nonce_res = std::move(nonce_res)] {
            ...
            auto built{platform::st::BuildDpnsPreorder(self->m_service.client().sdk(),
                                                       self->m_record.identity_id, *nonce_res.value + 1,
...
void IdentityFlow::broadcastDomain()
{
    ...
    m_service.client().getIdentityContractNonce(m_record.identity_id, platform::DPNS_CONTRACT_ID,
        ...
        self->m_service.post([self, nonce_res = std::move(nonce_res)] {
            ...
            auto built{platform::st::BuildDpnsDomain(self->m_service.client().sdk(),
                                                     rec.identity_id, *nonce_res.value + 1, rec.label,
