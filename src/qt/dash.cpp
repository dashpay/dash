...
{
    return QCoreApplication::translate("jast-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in jast.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in jast.qrc)
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(jast);
    Q_INIT_RESOURCE(jast_locale);
    ...
    /// 6. Determine availability of data directory and parse jast.conf
    ...
    // jast: links repeatedly have their payment requests routed to this process:
