...
void OverviewPage::coinJoinStatus(bool fForce)
{
    ...
    if (!fForce && clientModel->node().shutdownRequested()) return;
    ...
    const QString strCoinJoinName{QString::fromStdString(gCoinJoinName)};
    bool notMixing{true};
    if (!walletModel->withCoinJoin([&](auto& client) { notMixing = !client.isMixing(); })) return;
    ui->toggleCoinJoin->setText(notMixing ? tr("Start %1").arg(strCoinJoinName) : tr("Stop %1").arg(strCoinJoinName));

    if (!fForce && !clientModel->masternodeSync().isBlockchainSynced()) return;
