...
const char * const BITCOIN_CONF_FILENAME = "jast.conf";
const char * const BITCOIN_PID_FILENAME = "jastd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "jast" is a composite category enabling all Dash-related debug output
            if(ptrCategory->count(string("jast"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty jast.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override jast.conf
