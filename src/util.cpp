...
const char * const BITCOIN_CONF_FILENAME = "kriss.conf";
const char * const BITCOIN_PID_FILENAME = "krissd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "kriss" is a composite category enabling all Kriss-related debug output
            if(ptrCategory->count(string("kriss"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KrissCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KrissCore
    // Mac: ~/Library/Application Support/KrissCore
    // Unix: ~/.krisscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KrissCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KrissCore";
#else
    // Unix
    return pathRet / ".krisscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty kriss.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override kriss.conf
