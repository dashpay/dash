...
const char * const BITCOIN_CONF_FILENAME = "kliq.conf";
const char * const BITCOIN_PID_FILENAME = "kliqd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "kliq" is a composite category enabling all Kliq-related debug output
            if(ptrCategory->count(string("kliq"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KliqCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KliqCore
    // Mac: ~/Library/Application Support/KliqCore
    // Unix: ~/.kliqcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KliqCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KliqCore";
#else
    // Unix
    return pathRet / ".kliqcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty kliq.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override kliq.conf
