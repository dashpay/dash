...
const char * const BITCOIN_CONF_FILENAME = "ox.conf";
const char * const BITCOIN_PID_FILENAME = "oxd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ox" is a composite category enabling all Ox-related debug output
            if(ptrCategory->count(string("ox"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OxCore
    // Mac: ~/Library/Application Support/OxCore
    // Unix: ~/.oxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OxCore";
#else
    // Unix
    return pathRet / ".oxcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ox.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ox.conf
