...
const char * const BITCOIN_CONF_FILENAME = "gvidon.conf";
const char * const BITCOIN_PID_FILENAME = "gvidond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gvidon" is a composite category enabling all Gvidon-related debug output
            if(ptrCategory->count(string("gvidon"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GvidonCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GvidonCore
    // Mac: ~/Library/Application Support/GvidonCore
    // Unix: ~/.gvidoncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GvidonCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GvidonCore";
#else
    // Unix
    return pathRet / ".gvidoncore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gvidon.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gvidon.conf
