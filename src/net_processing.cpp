...
public:
    PeerManagerImpl(const CChainParams& chainparams, CConnman& connman, CAddrMan& addrman,
                    ...
                    CTxMemPool& pool, const std::unique_ptr<LLMQContext>& llmq_ctx, CGovernanceManager& gov_man, bool ignore_incoming_txs);
...
private:
    ...
    CGovernanceManager& m_governance_manager;
...
std::unique_ptr<PeerManager> PeerManager::make(const CChainParams& chainparams, CConnman& connman, CAddrMan& addrman, BanMan* banman,
                                               ...
                                               const std::unique_ptr<LLMQContext>& llmq_ctx, CGovernanceManager& gov_man, bool ignore_incoming_txs)
{
    return std::make_unique<PeerManagerImpl>(chainparams, connman, addrman, banman, scheduler, chainman, pool, llmq_ctx, gov_man, ignore_incoming_txs);
}
...
bool PeerManagerImpl::AlreadyHave(const CInv& inv)
{
    ...
    */
    ...
    case MSG_GOVERNANCE_OBJECT_VOTE:
        return !m_governance_manager.ConfirmInventoryRequest(inv);
...

    {
        ...
        while (it != peer.m_getdata_requests.end() && it->IsKnownType()) {
            ...
            if (!push && inv.type == MSG_GOVERNANCE_OBJECT) {
                ...
                bool topush = false;
                if (m_governance_manager.HaveObjectForHash(inv.hash)) {
                    ss.reserve(1000);
                    if (m_governance_manager.SerializeObjectForHash(inv.hash, ss)) {
                        topush = true;
                    }
                }
                if (topush) {
            ...
            if (!push && inv.type == MSG_GOVERNANCE_OBJECT_VOTE) {
                CDataStream ss(SER_NETWORK, pfrom.GetSendVersion());
                bool topush = false;
                if (m_governance_manager.HaveVoteForHash(inv.hash)) {
                    ss.reserve(1000);
                    if (m_governance_manager.SerializeVoteForHash(inv.hash, ss)) {
                        topush = true;
                    }
                }
                if (topush) {
    ...
    const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (found)
    {
        ...
        m_governance_manager.ProcessMessage(pfrom, *this, m_connman, msg_type, vRecv);
