...
UniValue ShimNetInfoPlatform(const T1& obj, const MnType& type)
{
    ...
    // Nothing to do here if not EvoNode or capable of natively storing Platform-specific fields
    if (type != MnType::Evo || obj.netInfo->CanStorePlatform()) return ret;

    CNetAddr addr{obj.netInfo->GetPrimary()};
    ret.pushKV(PurposeToString(Purpose::PLATFORM_HTTPS, /*lower=*/true).data(),
    ...
    if constexpr (!std::is_same_v<std::decay_t<T1>, CSimplifiedMNListEntry>) {
        ...
        ret.pushKV(PurposeToString(Purpose::PLATFORM_P2P, /*lower=*/true).data(),
...
uint16_t ShimPlatformPort(const T1& obj)
{
    ...
    if constexpr (is_p2p) {
        static_assert(!std::is_same_v<std::decay_t<T1>, CSimplifiedMNListEntry>, "CSimplifiedMNListEntry doesn't have plaformP2PPort");
        if (is_legacy) {
