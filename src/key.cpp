...
std::optional<std::array<uint8_t, CPubKey::COMPACT_SIGNATURE_SIZE>> CKey::SignCompact(const uint256 &hash) const {
    if (!fValid)
        return std::nullopt;
    std::array<uint8_t, CPubKey::COMPACT_SIGNATURE_SIZE> vchSig{};
    int rec = -1;
