                                                    ...
                                                    CCreditPool prev, const Consensus::Params& consensusParams)
{
    ...
    if (DeploymentActiveAt(*block_index, Params().GetConsensus(), Consensus::DEPLOYMENT_WITHDRAWALS2)) {
        currentLimit = std::max(CAmount(0), std::min(currentLimit, LimitAmountV23 - latelyUnlocked));
