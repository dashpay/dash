...
{
    AssertLockHeld(::cs_main);
    return CheckSpecialTxInner(nullptr, tx, pindexPrev, is_v24_active, view, std::nullopt, check_sigs,
    ...
    try {
        ...
        for (size_t i = 0; i < block.vtx.size(); ++i) {
            ...
            if (!CheckSpecialTxInner(&chain, *ptr_tx, pindex->pprev, is_v24_active, view, indexes, fCheckCbTxMerkleRoots,
                                     SpecialTxContext::Block, tx_state)) {
...
bool CheckProDisTx(const CTransaction& tx, gsl::not_null<const CBlockIndex*> pindexPrev,
                   CDeterministicMNManager& dmnman, const Consensus::Params& consensus_params, bool is_v24_active,
                   TxValidationState& state, bool check_sigs, SpecialTxContext context)
{
