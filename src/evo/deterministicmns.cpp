    ...
    for (int i = 1; i < (int)block.vtx.size(); i++) {
        ...
        if (tx.nType == TRANSACTION_PROVIDER_REGISTER) {
            ...
            if (!qc.commitment.IsNull()) {
                ...
                auto quorumBaseBlockIndex = pindexPrev->GetAncestor(quorumHeight);
                if (!quorumBaseBlockIndex || quorumBaseBlockIndex->GetBlockHash() != qc.commitment.quorumHash) {
                ...
                HandleQuorumCommitment(qc.commitment, quorumBaseBlockIndex, newList, debugLogs);
            }
        }
    }
