...
{
    try {
        const platform_ffi::FfiDpnsName decoded{sdk.decode_dpns_domain(ToSlice(doc))};
        DpnsName name;
...
{
    try {
        const platform_ffi::FfiProfile decoded{sdk.decode_dashpay_profile(ToSlice(doc))};
        Profile profile;
...
{
    try {
        const platform_ffi::FfiContactRequest decoded{sdk.decode_contact_request(ToSlice(doc))};
