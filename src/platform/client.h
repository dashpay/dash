...
public:
    ...
    //! Resolve an exact normalized label under the "dash" parent domain. An
    //! empty optional in the value means the name is proven absent.
    ...
    //! The underlying Platform SDK handle, for the DPP decoders and
    //! state-transition builders that need the network's protocol version.
    virtual const platform_ffi::PlatformClient& sdk() const = 0;
};

//! Create the production client: the Dash Platform SDK (dash-platform-cxx)
//! with its own DAPI transport, fed endpoints, quorum keys and ChainLock
//! heights from this node. `platform_llmq_type` is the LLMQ type Platform
//! quorums use on this network (Consensus::Params::llmqTypePlatform).
