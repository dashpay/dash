...
class DIP3Test(BitcoinTestFramework):
    ...
    def create_mn_protx(self, node, idx, alias):
        ...
        mn.protx_hash = node.protx('register', mn.collateral_address, '127.0.0.1:%d' % mn.p2p_port, mn.ownerAddr, mn.operatorAddr, mn.votingAddr, 0, mn.collateral_address)
    ...
    def test_fail_create_protx(self, node):
        ...
        blsKey = node.bls('generate')
        assert_raises_jsonrpc(None, "bad-tx-type", node.protx, 'register', address, '127.0.0.1:10000', key, blsKey['public'], key, 0, address)

    def test_success_create_protx(self, node):
        address = node.getnewaddress()
        key = node.getnewaddress()
        blsKey = node.bls('generate')
        txid = node.protx('register', address, '127.0.0.1:10000', key, blsKey['public'], key, 0, address)
