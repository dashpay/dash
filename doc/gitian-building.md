...
*Setup instructions for a Gitian build of Gvidon Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Gvidon
...
- [Building Gvidon Core](#building-gvidon-core)
...
Clone the git repositories for Gvidon Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Petr-Economissa/gvidon
```
...
in the Gvidon Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Gvidon Core
----------------

To build Gvidon Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Gvidon Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gvidon/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Petr-Economissa/gvidon
...
For example:
```bash
URL=https://github.com/crowning-/gvidon.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gvidon=${COMMIT} --url gvidon=${URL} ../gvidon/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gvidon=${COMMIT} --url gvidon=${URL} ../gvidon/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gvidon=${COMMIT} --url gvidon=${URL} ../gvidon/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gvidon git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gvidon/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/gvidon.git
SIGPATH=/some/root/path/gvidon-detached-sigs.git

./bin/gbuild --url gvidon=${BTCPATH},signature=${SIGPATH} ../gvidon/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gvidon-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gvidon-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/gvidon-osx-build.assert
```
...
[gvidon/gitian.sigs](https://github.com/gvidon/gitian.sigs/) repository, or if that's not possible to create a pull
request.
