...
- [Building Owncoin Core](#building-owncoin-core)
...
git clone https://github.com/dmrtsvetkov/owncoin
```
...
in the Owncoin Core repository under 'Fetch and build inputs' to install sources which require
...
```bash
    ...
    From https://github.com/dmrtsvetkov/owncoin
...
and the owncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../owncoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/owncoin.git
SIGPATH=/some/root/path/owncoin-detached-sigs.git

./bin/gbuild --url owncoin=${BTCPATH},signature=${SIGPATH} ../owncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/owncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/owncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/owncoin-osx-build.assert
```
...
[owncoin/gitian.sigs](https://github.com/dmrtsvetkov/gitian.sigs/) repository, or if that's not possible to create a pull
request.
