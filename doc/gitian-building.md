...
- [Building Dash Core](#building-jast-core)
...
git clone https://github.com/microcrm/jastcoin
```
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/jast/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/microcrm/jastcoin
...
For example:
```bash
URL=https://github.com/crowning-/jast.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit jast=${COMMIT} --url jast=${URL} ../jast/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit jast=${COMMIT} --url jast=${URL} ../jast/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit jast=${COMMIT} --url jast=${URL} ../jast/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the jast git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../jast/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/jast.git
SIGPATH=/some/root/path/jast-detached-sigs.git

./bin/gbuild --url jast=${BTCPATH},signature=${SIGPATH} ../jast/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/jast-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/jast-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/jast-osx-build.assert
```
...
[jast/gitian.sigs](https://github.com/jastcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
