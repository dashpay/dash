...
* While the field `service` (deprecated in dash#6665) can be re-enabled using `-deprecatedrpc=service`, its effective
  value can be obtained by querying `addresses['core_p2p'][0]`.
...
* The field `addresses` will now also report on platform P2P and platform HTTPS endpoints as `addresses['platform_p2p']`
  ...
  * On payloads prior to extended addresses, if a masternode update affects `platformP2PPort` and/or `platformHTTPPort`
